...
'''
Fossen / marine (SNAME) naming layer.
------------------------------------
This is NOT a second math implementation - it's the exact same DCM/quaternion
math above, just relabeled with the variable names Fossen's textbook and the
marine GNC literature use, so the {n}<->{b} transform reads naturally next to
aerospace NED<->body code.

Aerospace  -> Fossen
yaw,pitch,roll (Euler)      -> psi, theta, phi   (still Z-Y-X, still same DCM)
NED frame                   -> {n} frame
body frame                  -> {b} frame
[x,y,z,phi,theta,psi]       -> eta (pose vector, expressed in {n})
[u,v,w,p,q,r]               -> nu  (body-frame velocity vector)

R_n_b(eta) rotates a vector FROM {b} TO {n}, exactly like R_b^n above.
'''

def eta_to_R_n_b(eta):
    """
    Fossen-style wrapper: build the {b}->{n} rotation matrix from a pose
    vector eta = [x, y, z, phi, theta, psi].

    Identical math to euler_to_DCM - only the input packaging and variable
    names differ, since Fossen bundles position + attitude into one eta
    vector rather than passing angles separately.
    """
    _, _, _, phi, theta, psi = eta
    return euler_to_DCM(yaw=psi, pitch=theta, roll=phi)


def R_n_b_to_eta_angles(R, position=(0.0, 0.0, 0.0)):
    """
    Inverse of the above: DCM -> (phi, theta, psi), packaged back into a
    full eta vector given a position. Same math as dcm_to_euler.
    """
    psi, theta, phi = dcm_to_euler(R)
    x, y, z = position
    return np.array([x, y, z, phi, theta, psi])


def quat_to_R_n_b(q):
    """Fossen-labeled alias of quat_to_R - {b}->{n} rotation from a unit quaternion."""
    return quat_to_R(q)
